...
const COLUMN_ALIASES: Record<string, string[]> = {
  ...
  accountIdentifier: [
    ...
    'kontoiban',
    'auftraggeberkonto',
    'zahlungspflichtigerkonto',
    'empfaengerkonto',
    'zahlungsbeteiligterkonto',
  ],
  purpose: [
    'verwendungszweck',
...
export function parseBankCsv(text: string): ParseResult {
  ...
  for (let i = headerRowIndex + 1; i < rows.length; i++) {
    ...
    const accountIdentifier =
      cell(indices.accountIdentifier) ||
      // Einige Bankexporte liefern die Kontonummer ohne passende Spaltenüberschrift.
      // Dann trotzdem eine vorhandene IBAN aus der gesamten CSV-Zeile verwenden.
      row
        .join(' ')
        .match(/\b[A-Z]{2}\s?\d{2}(?:\s?[A-Z0-9]){10,30}\b/i)?.[0]
        ?.replace(/\s+/g, '')
        .toUpperCase()
