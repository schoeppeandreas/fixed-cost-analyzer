        ...
        {groups.map((group) => (
          <Card key={group.category.id}>
            <CardHeader className="gap-1">
              ...
              <CardDescription>
                {group.items.length} {group.items.length === 1 ? 'Serie' : 'Serien'} ·{' '}
                {group.items.reduce((count, item) => count + item.transactions.length, 0)}{' '}
                {group.items.reduce((count, item) => count + item.transactions.length, 0) === 1
                  ? 'Datensatz'
                  : 'Datensätze'} ·{' '}
